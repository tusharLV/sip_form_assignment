...
const SWPStartDate = ({ switchDate }) => {
  ...
  useEffect(() => {
    if (swpStartType !== '') {
      ...
      date =
        swpStartType === 'immediate'
          ? date.setDate(date.getDate() + 15)
          : startDuration !== ''
          ? date.setFullYear(date.getFullYear() + parseInt(startDuration))
          : '';
    ...
    <div className={`${styles.subFormContainer}`}>
      ...
      <div className={`${styles.inputContainer}`}>
        <div className="w-1/3 ">
          ...
          <span className="md:px-4 px-2">
            Immediate <span className="line-clamp-1">(after 15 days)</span>
          </span>
        </div>
        <div className="w-2/3">
          <input
      ...
      <div className={`${styles.inputContainer}`}>
        <div className="md:w-1/3 w-1/2">
          <label htmlFor="">{text.SWP_start_date_label}</label>
        </div>
